<%@ page contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
<%@ taglib prefix="c" uri="jakarta.tags.core" %>
<%@ taglib prefix="fn" uri="jakarta.tags.functions" %>
<%@ taglib prefix="fmt" uri="jakarta.tags.fmt" %>
<!DOCTYPE html><html lang="es">

<head>

    <meta charset="UTF-8">

    <meta name="viewport"
          content="width=device-width, initial-scale=1.0">

    <title>Perfil | Prado H, Antonio</title>

    <style>

        * {
            margin: 0;
            padding: 0;
            box-sizing: border-box;
        }

        body {
            font-family: Inter, Arial, Helvetica, sans-serif;
            background: #080b12;
            color: #ffffff;
            overflow-x: hidden;
        }

        /* =====================================================
           CONTENEDOR PRINCIPAL
           ===================================================== */

        .page {
            min-height: 100vh;

            background:
                    radial-gradient(
                            circle at 85% 5%,
                            rgba(66, 232, 255, .07),
                            transparent 25%
                    ),
                    radial-gradient(
                            circle at 45% 90%,
                            rgba(124, 255, 107, .05),
                            transparent 30%
                    );
        }

        .main {
            margin-left: 250px;
            min-height: 100vh;
        }

        /* =====================================================
           TOPBAR
           ===================================================== */

        .topbar {
            height: 82px;
            padding: 0 35px;

            display: flex;
            align-items: center;
            justify-content: space-between;

            background: rgba(8, 11, 18, .88);

            border-bottom:
                    1px solid rgba(255,255,255,.07);

            position: sticky;
            top: 0;
            z-index: 100;

            backdrop-filter: blur(15px);
        }

        .topbar-left {
            display: flex;
            align-items: center;
            gap: 18px;
        }

        .topbar-mark {
            width: 42px;
            height: 42px;

            display: flex;
            align-items: center;
            justify-content: center;

            border:
                    1px solid rgba(124,255,107,.4);

            color: #7cff6b;

            font-size: 18px;
            font-weight: 900;
        }

        .topbar-left h1 {
            font-size: 21px;
            font-weight: 800;
            letter-spacing: -.5px;
        }

        .topbar-left span {
            display: block;
            margin-top: 4px;

            color: #697587;
            font-size: 12px;
        }

        .admin {
            display: flex;
            align-items: center;
            gap: 13px;
        }

        .admin-info {
            text-align: right;
        }

        .admin-name {
            display: block;

            color: #ffffff;
            font-size: 13px;
            font-weight: 700;
        }

        .admin-role {
            display: block;
            margin-top: 4px;

            color: #697587;
            font-size: 11px;
        }

        .avatar {
            width: 43px;
            height: 43px;

            display: flex;
            align-items: center;
            justify-content: center;

            border-radius: 50%;

            background:
                    linear-gradient(
                            135deg,
                            #7cff6b,
                            #42e8ff
                    );

            color: #080b12;

            font-size: 15px;
            font-weight: 900;

            box-shadow:
                    0 0 25px rgba(124,255,107,.15);
        }

        /* =====================================================
           CONTENIDO
           ===================================================== */

        .content {
            padding: 45px 40px 70px;
            max-width: 1250px;
        }

        .header {
            margin-bottom: 35px;
        }

        .header-label {
            display: block;

            color: #7cff6b;

            font-size: 11px;
            font-weight: 800;

            letter-spacing: 4px;

            margin-bottom: 12px;
        }

        .header h2 {
            font-size: clamp(30px, 4vw, 46px);

            line-height: 1.05;

            letter-spacing: -2px;

            margin-bottom: 14px;
        }

        .header h2 span {
            color: #7cff6b;
        }

        .header p {
            max-width: 700px;

            color: #7d8999;

            font-size: 14px;
            line-height: 1.8;
        }

        /* =====================================================
           FORMULARIO
           ===================================================== */

        .form-container {

            background:
                    linear-gradient(
                            145deg,
                            #10151e,
                            #0c1017
                    );

            border:
                    1px solid rgba(255,255,255,.07);

            padding: 35px;

            position: relative;
        }

        .form-container::before {

            content: "";

            position: absolute;

            top: 0;
            left: 0;

            width: 65px;
            height: 2px;

            background:
                    linear-gradient(
                            90deg,
                            #7cff6b,
                            transparent
                    );
        }

        /* =====================================================
           SECCIONES
           ===================================================== */

        .seccion {

            margin-bottom: 40px;

            padding-bottom: 35px;

            border-bottom:
                    1px solid rgba(255,255,255,.06);
        }

        .seccion:last-of-type {
            border-bottom: none;
        }

        .seccion h2 {

            margin-bottom: 25px;

            color: #ffffff;

            font-size: 16px;
            font-weight: 800;

            display: flex;
            align-items: center;
            gap: 10px;
        }

        .seccion h2::before {

            content: "";

            width: 4px;
            height: 18px;

            background: #7cff6b;

            display: inline-block;
        }

        /* =====================================================
           FILAS
           ===================================================== */

        .fila {

            display: grid;

            grid-template-columns:
                    repeat(2, minmax(0, 1fr));

            gap: 22px;
        }

        /* =====================================================
           CAMPOS
           ===================================================== */

        .campo {
            margin-bottom: 22px;
        }

        .campo label {

            display: block;

            margin-bottom: 9px;

            color: #b7c0cc;

            font-size: 11px;

            font-weight: 700;

            text-transform: uppercase;

            letter-spacing: 1.2px;
        }

        .campo input,
        .campo textarea {

            width: 100%;

            padding: 14px 15px;

            border:
                    1px solid rgba(255,255,255,.09);

            border-radius: 0;

            background:
                    rgba(255,255,255,.025);

            color: #ffffff;

            font-family:
                    Inter,
                    Arial,
                    sans-serif;

            font-size: 13px;

            outline: none;

            transition:
                    border-color .3s ease,
                    background .3s ease,
                    box-shadow .3s ease;
        }

        .campo input::placeholder,
        .campo textarea::placeholder {

            color: #4f5b6b;
        }

        .campo input:focus,
        .campo textarea:focus {

            border-color:
                    rgba(124,255,107,.55);

            background:
                    rgba(124,255,107,.025);

            box-shadow:
                    0 0 0 3px
                    rgba(124,255,107,.04);
        }

        .campo textarea {

            min-height: 140px;

            resize: vertical;

            line-height: 1.7;
        }

        .campo small {

            display: block;

            margin-top: 8px;

            color: #596575;

            font-size: 10px;

            line-height: 1.6;
        }

        /* =====================================================
           FOTO
           ===================================================== */

        .foto-box {

            padding: 25px;

            border:
                    1px solid rgba(255,255,255,.07);

            background:
                    rgba(255,255,255,.015);
        }

        .foto-box input[type="file"] {

            padding: 12px;

            cursor: pointer;

            color: #8995a5;
        }

        .foto-box input[type="file"]::file-selector-button {

            margin-right: 12px;

            padding: 9px 13px;

            border: none;

            background:
                    rgba(124,255,107,.10);

            color: #7cff6b;

            cursor: pointer;

            font-size: 11px;

            font-weight: 700;
        }

        .foto-actual {

            margin-top: 25px;

            padding-top: 25px;

            border-top:
                    1px solid rgba(255,255,255,.06);
        }

        .foto-actual p {

            margin-bottom: 15px;

            color: #8995a5;

            font-size: 12px;
        }

        .foto-actual p strong {

            color: #ffffff;
        }

        .profile-preview {

            width: 150px;
            height: 150px;

            object-fit: cover;

            border-radius: 50%;

            border:
                    3px solid #7cff6b;

            display: block;

            box-shadow:
                    0 0 30px
                    rgba(124,255,107,.12);
        }

        /* =====================================================
           BOTONES
           ===================================================== */

        .botones {

            display: flex;

            align-items: center;

            gap: 12px;

            margin-top: 5px;
        }

        .btn {

            border: none;

            padding: 13px 22px;

            font-size: 12px;

            font-weight: 800;

            cursor: pointer;

            text-decoration: none;

            transition: .3s;

            letter-spacing: .3px;
        }

        .btn-guardar {

            background: #7cff6b;

            color: #080b12;
        }

        .btn-guardar:hover {

            background: #a0ff92;

            transform: translateY(-2px);

            box-shadow:
                    0 8px 25px
                    rgba(124,255,107,.15);
        }

        .btn-cancelar {

            background:
                    rgba(255,255,255,.06);

            color: #8995a5;

            border:
                    1px solid rgba(255,255,255,.08);
        }

        .btn-cancelar:hover {

            color: #ffffff;

            border-color:
                    rgba(255,255,255,.2);

            transform: translateY(-2px);
        }

        /* =====================================================
           FOOTER
           ===================================================== */

        .footer {

            margin-top: 30px;

            padding-top: 22px;

            border-top:
                    1px solid rgba(255,255,255,.06);

            display: flex;

            align-items: center;

            justify-content: space-between;

            color: #465161;

            font-size: 10px;
        }

        .footer strong {
            color: #7cff6b;
        }

        /* =====================================================
           RESPONSIVE
           ===================================================== */

        @media (max-width: 900px) {

            .main {
                margin-left: 200px;
            }

            .content {
                padding: 30px 25px 50px;
            }

            .fila {
                grid-template-columns: 1fr;
                gap: 0;
            }

        }

        @media (max-width: 600px) {

            .main {
                margin-left: 0;
            }

            .topbar {
                height: 70px;
                padding: 0 18px;
            }

            .topbar-mark {
                display: none;
            }

            .topbar-left h1 {
                font-size: 18px;
            }

            .topbar-left span {
                display: none;
            }

            .admin-info {
                display: none;
            }

            .content {
                padding: 25px 16px 45px;
            }

            .form-container {
                padding: 22px;
            }

            .header h2 {
                font-size: 32px;
            }

            .botones {
                flex-direction: column;
                align-items: stretch;
            }

            .btn {
                text-align: center;
            }

            .footer {
                flex-direction: column;
                align-items: flex-start;
                gap: 8px;
            }

        }

    </style>

    <link rel="stylesheet"
          href="${pageContext.request.contextPath}/css/admin.css">

</head>

<body>

<div class="page">

    <!-- =====================================================
         SIDEBAR
         ===================================================== -->

    <c:set var="activePage" value="perfil" scope="request" />
    <jsp:include page="/WEB-INF/jsp/fragments/sidebar.jsp" />


    <!-- =====================================================
         CONTENIDO PRINCIPAL
         ===================================================== -->

    <main class="main">


        <!-- =================================================
             TOPBAR
             ================================================= -->

        <header class="topbar">

            <div class="topbar-left">

                <div class="topbar-mark">AP</div>

                <div>

                    <h1>
                        Mi perfil
                    </h1>

                    <span>
                        Información personal · Portafolio
                    </span>

                </div>

            </div>


            <div class="admin">

                <div class="admin-info">

                    <span class="admin-name">
                        Antonio
                    </span>

                    <span class="admin-role">
                        Administrador
                    </span>

                </div>

                <div class="avatar">AP</div>

            </div>

        </header>


        <!-- =================================================
             CONTENIDO
             ================================================= -->

        <section class="content">


            <!-- ENCABEZADO -->

            <div class="header">

                <span class="header-label">
                    CONFIGURACIÓN DEL PERFIL
                </span>

                <h2>
                    Mi información
                    <span>personal.</span>
                </h2>

                <p>
                    Administra la información que aparecerá
                    públicamente en tu portafolio. Los cambios
                    realizados aquí se reflejarán automáticamente
                    en la página principal.
                </p>

            </div>


            <!-- =================================================
                 FORMULARIO
                 ================================================= -->

            <div class="form-container">

                <form
                        action="${pageContext.request.contextPath}/admin/perfil/guardar"
                        method="post"
                        enctype="multipart/form-data">

                    <input type="hidden"
                           name="${_csrf.parameterName}"
                           value="${_csrf.token}">


                    <!-- ID -->

                    <input
                            type="hidden"
                            name="id" value="${perfil.id}">


                    <!-- =================================================
                         INFORMACIÓN PERSONAL
                         ================================================= -->

                    <div class="seccion">

                        <h2>
                            Información personal
                        </h2>


                        <div class="fila">

                            <!-- NOMBRES -->

                            <div class="campo">

                                <label for="nombre">
                                    Nombres
                                </label>

                                <input
                                        type="text"
                                        id="nombre"
                                        name="nombre" value="${perfil.nombre}"
                                        placeholder="Ejemplo: Antonio"
                                        required>

                            </div>


                            <!-- APELLIDOS -->

                            <div class="campo">

                                <label for="apellidos">
                                    Apellidos
                                </label>

                                <input
                                        type="text"
                                        id="apellidos"
                                        name="apellidos" value="${perfil.apellidos}"
                                        placeholder="Ejemplo: Prado H"
                                        required>

                            </div>

                        </div>


                        <div class="fila">

                            <!-- CARRERA -->

                            <div class="campo">

                                <label for="carrera">
                                    Carrera
                                </label>

                                <input
                                        type="text"
                                        id="carrera"
                                        name="carrera" value="${perfil.carrera}"
                                        placeholder="Diseño y Programación Web"
                                        required>

                            </div>


                            <!-- INSTITUTO -->

                            <div class="campo">

                                <label for="instituto">
                                    Instituto
                                </label>

                                <input
                                        type="text"
                                        id="instituto"
                                        name="instituto" value="${perfil.instituto}"
                                        placeholder="Instituto Andrés Avelino Cáceres Dorregaray"
                                        required>

                            </div>

                        </div>

                    </div>


                    <!-- =================================================
                         PRESENTACIÓN
                         ================================================= -->

                    <div class="seccion">

                        <h2>
                            Presentación
                        </h2>


                        <!-- DESCRIPCIÓN -->

                        <div class="campo">

                            <label for="descripcion">
                                Descripción corta
                            </label>

                            <textarea
                                    id="descripcion"
                                    name="descripcion"
                                    placeholder="Escribe una breve presentación sobre ti..."
                                    required>${perfil.descripcion}</textarea>

                        </div>


                        <!-- SOBRE MÍ -->

                        <div class="campo">

                            <label for="sobreMi">
                                Sobre mí
                            </label>

                            <textarea
                                    id="sobreMi"
                                    name="sobreMi"
                                    placeholder="Cuéntanos más sobre ti, tus intereses, objetivos y experiencia...">${perfil.sobreMi}</textarea>

                        </div>

                    </div>


                    <!-- =================================================
                         CONTACTO
                         ================================================= -->

                    <div class="seccion">

                        <h2>
                            Información de contacto
                        </h2>


                        <div class="fila">


                            <!-- CORREO -->

                            <div class="campo">

                                <label for="correo">
                                    Correo electrónico
                                </label>

                                <input
                                        type="email"
                                        id="correo"
                                        name="correo" value="${perfil.correo}"
                                        placeholder="correo@ejemplo.com">

                            </div>


                            <!-- TELÉFONO -->

                            <div class="campo">

                                <label for="telefono">
                                    Teléfono
                                </label>

                                <input
                                        type="text"
                                        id="telefono"
                                        name="telefono" value="${perfil.telefono}"
                                        placeholder="+51 999 999 999">

                            </div>

                        </div>

                    </div>


                    <!-- =================================================
                         REDES
                         ================================================= -->

                    <div class="seccion">

                        <h2>
                            Redes profesionales
                        </h2>


                        <div class="fila">


                            <!-- GITHUB -->

                            <div class="campo">

                                <label for="github">
                                    GitHub
                                </label>

                                <input
                                        type="url"
                                        id="github"
                                        name="github" value="${perfil.github}"
                                        placeholder="https://github.com/usuario">

                            </div>


                            <!-- LINKEDIN -->

                            <div class="campo">

                                <label for="linkedin">
                                    LinkedIn
                                </label>

                                <input
                                        type="url"
                                        id="linkedin"
                                        name="linkedin" value="${perfil.linkedin}"
                                        placeholder="https://linkedin.com/in/usuario">

                            </div>

                        </div>

                    </div>


                    <!-- =================================================
                         FOTO DE PERFIL
                         ================================================= -->

                    <div class="seccion">

                        <h2>
                            Foto de perfil
                        </h2>


                        <div class="foto-box">


                            <!-- SELECCIONAR FOTO -->

                            <div class="campo">

                                <label for="archivoFoto">
                                    Seleccionar nueva foto
                                </label>

                                <input
                                        type="file"
                                        id="archivoFoto"
                                        name="archivoFoto"
                                        accept="image/jpeg,image/png,image/gif,image/webp">

                                <small>
                                    Selecciona una imagen desde tu
                                    computadora. Formatos permitidos:
                                    JPG, PNG, GIF o WEBP.
                                </small>

                            </div>


                            <!-- FOTO ACTUAL -->

                            <div
                                    class="foto-actual"
                                    style="${not empty perfil.foto ? '' : 'display:none'}">

                                <p>
                                    <strong>
                                        Foto actual
                                    </strong>
                                </p>

                                <img
                                        class="profile-preview"
                                        src="${pageContext.request.contextPath}/publico/imagen/${perfil.foto}"
                                        alt="${perfil.nombre} ${perfil.apellidos}">

                            </div>

                        </div>

                    </div>


                    <!-- =================================================
                         BOTONES
                         ================================================= -->

                    <div class="botones">

                        <button
                                type="submit"
                                class="btn btn-guardar">

                            Guardar cambios

                        </button>


                        <a
                                href="${pageContext.request.contextPath}/admin"
                                class="btn btn-cancelar">

                            Cancelar

                        </a>

                    </div>


                </form>

            </div>


            <!-- =================================================
                 FOOTER
                 ================================================= -->

            <div class="footer">

                <span>
                    PORTAFOLIO · ADMIN PANEL · 2026
                </span>

                <span>

                    <strong>
                        Prado H
                    </strong>

                    · Antonio

                </span>

            </div>


        </section>

    </main>

</div>

</body>

</html>








